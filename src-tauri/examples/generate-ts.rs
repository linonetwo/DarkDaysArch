...
fn main() {
  ...
  let ts_file_path = Path::join(&get_project_root().unwrap(), "../src/types/cdda/mapgen.ts");
  ...
  let generated_ts_content = String::from_utf8(buf).unwrap();
  let ts_file_path = Path::join(&get_project_root().unwrap(), "../src/types/cdda/palette.ts");
  // println!("{}", ts_file_path.into_os_string().into_string().unwrap());
  write(ts_file_path, generated_ts_content).expect("can not write ts file");

  let mut buf = Vec::new();
  write_definition_file::<_, furniture_json::CDDAFurnArray>(&mut buf, Default::default()).unwrap();
  let generated_ts_content = String::from_utf8(buf).unwrap();
  let ts_file_path = Path::join(&get_project_root().unwrap(), "../src/types/cdda/furniture.ts");
  // println!("{}", ts_file_path.into_os_string().into_string().unwrap());
  write(ts_file_path, generated_ts_content).expect("can not write ts file");
}
